...
contract BoringVaultDepositor is IDepositor {
     ...
     * @param vault The address of the BoringVault's associated teller
    ...
    function deposit(address vault, bytes depositPayload) public {
        ...
        boringVault = vault.call(abi.encodeWithSelector(selector));

        bytes4 selector = bytes4(keccak256(bytes("accountant()")));
        accountant = vault.call(abi.encodeWithSelector(selector));
        ...
        Asset asset = vault.call(abi.encodeWithSelector(selector, depositAsset));
