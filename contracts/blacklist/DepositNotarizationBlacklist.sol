...
{
    bytes32 public constant ADD_BLOCKLIST_ROLE =
        keccak256("ADD_BLOCKLIST_ROLE");
    bytes32 public constant REMOVE_BLOCKLIST_ROLE =
        keccak256("REMOVE_BLOCKLIST_ROLE");
    ...
    ) external onlyRole(ADD_BLOCKLIST_ROLE) {
    ...
    function removeFromBlacklist(
        bytes32 txId,
        uint32[] calldata vouts
    ) external onlyRole(REMOVE_BLOCKLIST_ROLE) {
        for (uint i = 0; i < vouts.length; i++) {
