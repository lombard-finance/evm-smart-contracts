...
describe('Consortium', function () {
  ...
  before(async function () {
    ...
    lombard = await deployContract<Consortium>('Consortium', [deployer.address]);
  ...
  it('checkProof :: should revert if not validator set is set', async function () {
    ...
    await expect(lombard.checkProof(ethers.randomBytes(32), '0x')).to.be.revertedWithCustomError(
      lombard,
      'NoValidatorSet'
    );
  });
  ...
  it('setNextValidatorSet :: should revert if no initial validator set', async function () {
    ...
    await expect(lombard.setNextValidatorSet(payload, proof)).to.be.revertedWithCustomError(lombard, 'NoValidatorSet');
  });

  describe('With Initial ValidatorSet', function () {
    beforeEach(async function () {
      const initialValset = getPayloadForAction(
        [10, [signer3.publicKey, signer1.publicKey, signer2.publicKey], [1, 1, 1], 2, 1],
        NEW_VALSET
      );
    ...
    it('should fail to set initial validator set again', async function () {
      const payload = getPayloadForAction([11, [signer1.publicKey], [1], 1, 1], NEW_VALSET);
      await expect(lombard.setInitialValidatorSet(payload)).to.revertedWithCustomError(lombard, 'ValSetAlreadySet');
    });
    ...
    it('should fail if epoch is not increasing', async function () {
      ...
      await expect(lombard.setNextValidatorSet(data.payload, data.proof)).to.be.revertedWithCustomError(
        lombard,
        'InvalidEpoch'
      );
    });
    ...
    describe('Signature verification', function () {
      ...
      it('should not succeed on invalid signatures', async function () {
        ...
        await expect(lombard.checkProof(ethers.sha256(payload), data.proof)).to.be.revertedWithCustomError(
          lombard,
          'NotEnoughSignatures'
        );
      });
