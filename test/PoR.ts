...
describe('PoR', function () {
    ...
    beforeEach(async () => {
        ...
        const contract = await upgrades.deployProxy(PoRFactory, [
    ...
    describe('Pubkey operations', function () {
        ...
        it('should revert if pubkey is invalid', async function () {
            await expect(
                por.connect(owner).addRootPubkey(ethers.randomBytes(64))
            ).to.be.revertedWithCustomError(por, 'InvalidRootPubkey');
        });
        ...
        it('should revert if root pubkey cannot be deleted', async function () {
            ...
            await expect(
                por.connect(owner).deleteRootPubkey(key)
            ).to.be.revertedWithCustomError(por, 'RootPubkeyCannotBeDeleted');
        });
        ...
        it('should revert if not called by owner', async function () {
            await expect(
                por.connect(operator).addRootPubkey(ethers.randomBytes(65))
            )
            ...
            await expect(
                por.connect(operator).deleteRootPubkey(ethers.randomBytes(65))
            )
                .to.be.revertedWithCustomError(
                    por,
                    'AccessControlUnauthorizedAccount'
    ...
    describe('Addresses operations', function () {
        ...
        describe('Add Addresses', function () {
            it('should add addresses correctly', async function () {
                ...
                const [rootPubkeyId, messages, signatures] =
                    await por.getPoRSignatureMessages([
                        '0xAddress1',
                        '0xAddress2',
                    ]);
                expect(rootPubkeyId).to.deep.equal([
        ...
        describe('Delete Addresses', function () {
            ...
            it('should ignore non-existing addresses', async function () {
                await por
                    .connect(owner)
                    .deleteAddresses(['0xNonExistingAddress']);
                const [rootPubkeyId, messages, signatures] =
                    await por.getPoRSignatureMessages([
                        '0xAddress1',
                        '0xAddress2',
            ...
            it('should revert if not called by owner', async function () {
                await expect(
                    por.connect(operator).deleteAddresses(['0xAddress1'])
                )
                    .to.be.revertedWithCustomError(
                        por,
                        'AccessControlUnauthorizedAccount'
        ...
        describe('Update Message and Signature', function () {
            ...
            it('should update messages and signatures correctly', async function () {
                ...
                const [rootPkId, messages, signatures] =
                    await por.getPoRSignatureMessages(['0xAddress1']);
        ...
        describe('Getters', function () {
            ...
            it('should return the correct values if addresses are provided', async function () {
                const [rootPkIds, messages, signatures] =
                    await por.getPoRSignatureMessages([
                        '0xAddress1',
                        '0xAddress2',
                        '0xAddress3',
            ...
            it('should return proper values in a range', async function () {
                const [addresses, rootPkIds, messages, signatures] =
                    await por.getPoRAddressSignatureMessages(1, 2);
            ...
            it('should return all values if no range is provided', async function () {
                ...
                expect(data.map((d: any) => d.addressStr)).to.deep.equal([
                    '0xAddress1',
                    '0xAddress2',
                    '0xAddress3',
                ]);
                ...
                expect(
                    data.map((d: any) => d.messageOrDerivationData)
                ).to.deep.equal(['Message1', 'Message2', 'Message3']);
            ...
            it('should return empty arrays if start is greater than end', async function () {
                const [addresses, rootPkIds, messages, signatures] =
                    await por.getPoRAddressSignatureMessages(2, 1);
