...
describe("PoR", function () {
  ...
  describe("Pubkey operations", function () {
    ...
    it("should revert if pubkey is invalid", async function () {
      await expect(por.connect(owner).addRootPubkey(ethers.randomBytes(64)))
    ...
    it("should revert if root pubkey cannot be deleted", async function () {
      const key = ethers.randomBytes(65);
      await por.connect(owner).addRootPubkey(key);
      await por.connect(operator).addAddresses(["0xAddress1"], [ethers.keccak256(key)], ["Message1"], [ethers.keccak256("0x01")]);
  ...
  describe("Addresses operations", function () {
    ...
    describe("Getters", function () {
      ...
      it("should return the correct values if addresses are provided", async function () {
        const [rootPkIds, messages, signatures] = await por.getPoRSignatureMessages(["0xAddress1", "0xAddress2", "0xAddress3"]);
        expect(rootPkIds).to.deep.equal([ethers.keccak256(pubkey1), ethers.keccak256(pubkey2), ethers.keccak256(pubkey3)]);
      ...
      it("should return all values if no range is provided", async function () {
        ...
        expect(data.map((d: any) => d.rootPkId)).to.deep.equal([ethers.keccak256(pubkey1), ethers.keccak256(pubkey2), ethers.keccak256(pubkey3)]);
