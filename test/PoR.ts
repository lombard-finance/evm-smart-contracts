...
describe("PoR", function () {
  ...
  describe("Pubkey operations", function () {
    it("should add root pubkey correctly", async function () {
      await por.connect(owner).addRootPubkey("a", "0xPubkey1");
    });

    it("should revert if root pubkey already exists", async function () {
      await por.connect(owner).addRootPubkey("a", "0xPubkey1");

      await expect(por.connect(owner).addRootPubkey("b", "0xPubkey1"))
        .to.be.revertedWithCustomError(por, "RootPubkeyAlreadyExists")
        .withArgs("0xPubkey1");
    });

    it("should revert if id already exists", async function () {
      await por.connect(owner).addRootPubkey("a", "0xPubkey2");

      await expect(por.connect(owner).addRootPubkey("a", "0xPubkey3"))
        .to.be.revertedWithCustomError(por, "IdAlreadyExists")
        .withArgs("a");
    });

    it("should delete root pubkey correctly", async function () {
      await por.connect(owner).addRootPubkey("a", "0xPubkey1");
      await por.connect(owner).deleteRootPubkey("0xPubkey1");
    });

    it("should revert if root pubkey cannot be deleted", async function () {
      await por.connect(owner).addRootPubkey("a", "0xPubkey1");
      await por.connect(operator).addAddresses(["0xAddress1"], ["a"], ["Message1"], [ethers.keccak256("0x01")]);
      await expect(por.connect(owner).deleteRootPubkey("0xPubkey1"))
    ...
    it("should revert if not called by owner", async function () {
      await expect(por.connect(operator).addRootPubkey("a", "0xPubkey1"))
      ...
      await expect(por.connect(operator).deleteRootPubkey("0xPubkey1"))
        .to.be.revertedWithCustomError(por, "AccessControlUnauthorizedAccount")
        .withArgs(await operator.getAddress(), await por.DEFAULT_ADMIN_ROLE());
    });
  });
  ...
  describe("Addresses operations", function () {
    beforeEach(async () => {
      ...
      await por.connect(owner).addRootPubkey("a", "0xPubkey1");
      await por.connect(owner).addRootPubkey("b", "0xPubkey2");
      await por.connect(owner).addRootPubkey("c", "0xPubkey3");
      await por.connect(owner).addRootPubkey("d", "0xPubkey4");
    });
