...
describe('VerifySignatureTest', function () {
    ...
    before(async function () {
        const VerifySignatureTest = await ethers.getContractFactory(
            'VerifySignatureTest'
        );
        verifySignatureTest =
            (await VerifySignatureTest.deploy()) as VerifySignatureTest;
    ...
    it('should verify MAX_PLAYERS constant value and its calculation', async function () {
        ...
        const tx = await verifySignatureTest.verifySignatureWithGas.staticCall(
            signer.address,
            messageHash,
            signature
        );
        ...
        const recoveredAddress = await verifySignatureTest.recoverSigner(
            messageHash,
            signature
        );
        ...
        const maxSignatures = Math.floor(
            Number(currentBlockGasLimit) / ecdsaVerificationGas
        );
        ...
        const requiredSignatures =
            Math.floor((Number(MAX_PLAYERS) * 2) / 3) + 1;
