...
describe("CBBTCPMM", function () {
  ...
  before(async function () {
    [deployer, withdrawalAddress, signer1, signer2, operator, pauser] =
  ...
  describe("Access Control", function () {
    it("should revert if withdrawal address is not set by the admin", async function () {
      await expect(
        pmm.connect(signer1).setWithdrawalAddress(await signer1.getAddress())
      )
    ...
    it("should set the stake limit", async function () {
      await expect(pmm.connect(operator).setStakeLimit(100))
    ...
    it("should fail to set the relative fee if over max commission", async function () {
      ...
      await expect(pmm.setRelativeFee(10001)).to.be.revertedWithCustomError(
        iface,
        "BadCommission"
      );
    });
  ...
  describe("Operations", function () {
    ...
    it("should fail to swap if PMM is not whitelisted as minter", async function () {
      await cbbtc
        .connect(signer1)
        .approve(await pmm.getAddress(), ethers.parseUnits("1", 8));
      await expect(
        pmm.connect(signer1).swapCBBTCToLBTC(ethers.parseUnits("1", 8))
      )
    ...
    describe("With whitelisted minter", function () {
      ...
      it("should allow swaps", async function () {
        await cbbtc
          .connect(signer1)
          .approve(await pmm.getAddress(), ethers.parseUnits("11", 8));
        await expect(
          pmm.connect(signer1).swapCBBTCToLBTC(ethers.parseUnits("11", 8))
        )
          .to.emit(cbbtc, "Transfer")
          .withArgs(
            signer1.address,
            await pmm.getAddress(),
            ethers.parseUnits("11", 8)
          )
        ...
        expect(await cbbtc.balanceOf(signer1.address)).to.equal(
          ethers.parseUnits("89", 8)
        );
        expect(await cbbtc.balanceOf(await pmm.getAddress())).to.equal(
          ethers.parseUnits("11", 8)
        );
      });
      ...
      it("should fail to swap more than limit", async function () {
        await cbbtc
          .connect(signer1)
          .approve(await pmm.getAddress(), ethers.parseUnits("130", 8));
        await expect(
          pmm.connect(signer1).swapCBBTCToLBTC(ethers.parseUnits("130", 8))
      ...
      it("should allow more swaps if limit is increased", async function () {
        await cbbtc
          .connect(signer1)
          .approve(await pmm.getAddress(), ethers.parseUnits("30", 8));
        ...
        await cbbtc
          .connect(signer2)
          .approve(await pmm.getAddress(), ethers.parseUnits("10", 8));
      ...
      it("should allow withdrawals", async function () {
        ...
        expect(
          await cbbtc.balanceOf(await withdrawalAddress.getAddress())
        ).to.equal(1);
      ...
      describe("When Paused", function () {
        ...
        it("should revert swaps", async function () {
          ...
          await expect(
            pmm.connect(signer1).swapCBBTCToLBTC(30)
          ).to.be.revertedWithCustomError(pmm, "EnforcedPause");
        });

        it("should revert withdrawals", async function () {
          await expect(pmm.withdrawCBBTC(1)).to.be.revertedWithCustomError(
            pmm,
            "EnforcedPause"
          );
        });
