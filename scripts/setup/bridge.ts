...
task('setup-add-destination', 'Call `addDestination` on bridge smart-contract')
  ...
  .addParam('adapter', 'The address of adapter', '0x0000000000000000000000000000000000000000')
  ...
  .setAction(async (taskArgs, hre, network) => {
    const { target, chainId, contract, relCommission, absCommission, adapter, requireConsortium, populate } = taskArgs;
    ...
    const toContract =
      contract.length != 42 ? contract : hre.ethers.AbiCoder.defaultAbiCoder().encode(['address'], [contract]);

    const bridge = await hre.ethers.getContractAt('Bridge', target);

    const existingDest = await bridge.getDestination(toChainId);
    if (existingDest.bridgeContract != hre.ethers.zeroPadValue(hre.ethers.ZeroAddress, 32)) {
      console.log(`Removing existing destination ${existingDest}`);
      if (populate) {
        const txData = await bridge.removeDestination.populateTransaction(toChainId);
        console.log(`removeDestination: ${JSON.stringify(txData, null, 2)}`);
      } else {
...
task('setup-bridge-rate-limits', 'Set rate limits')
  ...
  .setAction(async (taskArgs, hre, network) => {
    ...
    if (populate) {
      const txData = await bridgeContract.setRateLimits.populateTransaction(
        [
          {
            chainId: toChainId,
            limit: limit,
