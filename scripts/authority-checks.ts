...
task("authority-check", "Given the set of contracts connected to our services, checks the ownership of such contracts is consistent with our policies")
    .addParam("chainConf", "The JSON file containing contracts information", "mainnet.json")
    .setAction(async (taskArgs, hre) => {
        ...
        for (const chainLabel of Object.keys(contractsData)) {
            if (!(chainLabel in hre.config.networks)) {
                console.log(`Chain ${chainLabel} is not configured in Hardhat. Skipping.`);
                continue;
            }
            const provider = new hre.ethers.JsonRpcProvider(hre.config.networks[chainLabel].url);
            const chainContractsData = contractsData[chainLabel];
            console.log(`## Chain: ${chainLabel} - ${chainContractsData['chainId']}`);
            ...
            for (const adminLabel of Object.keys(chainContractsData["admin"])) {
                policyAddresses[chainContractsData["admin"][adminLabel]] = adminLabel;
            }
            ...
            for (const contractsGroupLabel of Object.keys(chainContractsData)) {
                ...
                for (const contractLabel of Object.keys(contractsGroup)) {
                    ...
                    if (!hre.ethers.isAddress(contractAddress)) {
                        console.log(`Skipped ${contractLabel}:${contractAddress}`);
                        continue;
                    }
                    try {
                        ...
                        if (ownerQueriedAddress.length < 22) {
                            console.log(`- ${contractLabel} -> this is not a contract`)
                            continue;
                        }
                        // Convert 32 byte address to a 20 byte checksummed hex
                        const ownerAddress = hre.ethers.getAddress("0x" + ownerQueriedAddress.slice(26));
                        // Lookup for known addresses
                        if (ownerAddress in policyAddresses) {
                            console.log(`- ${contractLabel} -> ${policyAddresses[ownerAddress]} OK`)
                        } else {
                            console.log(`- ${contractLabel} -> UNKNOWN ${ownerAddress}`);
                        }
                    } catch (e: any) {
                        if (e.info.error.code == -32000) {
                            console.log(`Error accessing ownership information for ${contractLabel}:${contractAddress}. WIP: AccessControl`);
                        } else {
                            console.log(`Unhandled error on ${contractLabel}:${contractAddress}`);
                        }
                    }
                }
            }
