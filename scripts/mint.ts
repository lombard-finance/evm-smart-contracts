...
async function main() {
  const lbtc = await ethers.getContractAt('LBTC', '0xD27cdA6E1eD6C807280670Ea0E06D1342f778B3E');
  ...
  const tx = await lbtc.mint(`0x${data.toString('hex')}`, `0x${proofSignature.toString('hex')}`);
  console.log(tx);
}
