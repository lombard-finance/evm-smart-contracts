...
task('deploy-voucher', 'Deploys the IBCVoucher contract')
  ...
  .setAction(async (taskArgs, hre, network) => {
    const { ledgerNetwork, lbtc, admin, treasury, fee, proxyFactoryAddr } = taskArgs;

    await create3('IBCVoucher', [lbtc, admin, fee, treasury], proxyFactoryAddr, ledgerNetwork, admin, hre);
  });
