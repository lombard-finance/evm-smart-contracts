 ...
 */
task("deploy-token-pool", "Deploys chainlink TokenPool contract")
  .addParam("router", "The chainlink ccip router")
  .addParam("lbtc", "The address of the LBTC contract / token to be used in the pool")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const pool = await hre.ethers.deployContract(
      "TokenPool",
      [adapter, lbtc, allowlist, rmn, router]
    );
