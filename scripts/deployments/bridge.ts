...
task("deploy-bridge", "Deploys the Bridge contract via create3")
  ...
  .setAction(async (taskArgs, hre) => {
    let { ledgerNetwork, lbtc, admin, proxyFactoryAddr, treasury, adapter, setBridge } = taskArgs;
    ...
    if(setBridge) {
      const adapterContract = await hre.ethers.getContractAt("AbstractAdapter", adapter);
