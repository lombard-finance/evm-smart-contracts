...
task('deploy-timelock', 'Deploys the LombardTimeLock contract')
  ...
  .setAction(async (taskArgs, hre) => {
    const { executors: executorsArg, proposers: proposersArg, minDelay } = taskArgs;

    let executors = executorsArg.split(',').filter((v: string) => v.length > 0) || [];
    ...
    let proposers = proposersArg.split(',').filter((v: string) => v.length > 0) || [];
    ...
    const timelock = await hre.ethers.deployContract('LombardTimeLock', constructorArguments);
