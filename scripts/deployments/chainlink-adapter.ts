...
function chainlinkAdapterTask(taskName: string) {
  task(taskName, 'Deploys the TokenPoolAdapter contract')
    ...
    .addVariadicPositionalParam('allowlist', 'The list of addresses allowed to bridge', [])
    .addParam('gasLimit', 'Execution gas limit on destination chain', 300_000n.toString())
    .setAction(async (taskArgs, hre) => {
      const { admin: adminArg, bridge, router, rmn, allowlist, gasLimit } = taskArgs;
      ...
      await verify(hre.run, await adapter.tokenPool(), {
        constructorArguments: [await adapter.lbtc(), router, allowlist, rmn, await adapter.getAddress()]
        // force: true,
      });

      const tokenPool = await hre.ethers.getContractAt('LombardTokenPool', await adapter.tokenPool());

      await tokenPool.acceptOwnership();
      console.log('TokenPool ownership accepted:', await tokenPool.owner());

      await sleep(12_000);

      if (admin && (await adapter.owner()).toLowerCase() != admin.toLowerCase()) {
        console.log(admin, await adapter.owner());
        await adapter.transferOwnership(admin);
      }

      if (admin && (await tokenPool.owner()).toLowerCase() != admin.toLowerCase()) {
