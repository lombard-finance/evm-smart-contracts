...
task('blacklist', 'Add or remove the transaction output to/from the blacklist contract')
  .addPositionalParam('txHash', 'the account to set as initial default admin of the blacklist contract')
  .addPositionalParam('vout', 'the account in charge of contract upgrades to set as admin of the proxy')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const blacklistContractFactory = await hre.ethers.getContractFactory('DepositNotarizationBlacklist');
    const blacklistContract = blacklistContractFactory.attach(address) as DepositNotarizationBlacklist;
    ...
    if (remove) {
      try {
        ...
        console.error(error);
        console.error(`Could not verify current status of ${txHash}:${vout}. Aborted.`);
        return;
      }
