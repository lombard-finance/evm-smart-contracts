...
export class CustomRuntimeEnvironment {
  ...
  async getTimelock(address: string) {
    const timelock = await this.hre.ethers.getContractAt('ITimelockController', address);
  ...
  async getProxyAdmin(proxy: string) {
    ...
    console.log(`ProxyAdmin of ${proxy} found at ${await proxyAdmin.getAddress()}`);
  ...
  async deployImplementation(contract: string): Promise<string> {
    ...
    console.log(`New implementation of ${contract} deployed at ${addr}, verifying...`);
  ...
  async deployTransparentProxy(admin: string, implementation: string, initData: BytesLike): Promise<string> {
    ...
    console.log(`New transparent upgradeable proxy for ${implementation} deployed at ${proxyAddress}`);
    return proxyAddress;
  }
}
