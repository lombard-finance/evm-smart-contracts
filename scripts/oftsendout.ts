...
task('send-oft-bera', 'Sends LBTC from mainnet to bera').setAction(async (taskArgs, hre) => {
  const [signer] = await hre.ethers.getSigners();
  let owner = await signer.getAddress();

  const adapter = await hre.ethers.getContractAt('LBTCOFTAdapter', '0x1290A6b480f7eF14925229fdB66f5680aD8F44AD');
  const lbtc = await hre.ethers.getContractAt('LBTC', '0x8236a87084f8B84306f72007F36F2618A5634494');
