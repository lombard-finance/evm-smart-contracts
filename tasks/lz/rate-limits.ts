...
export const rateLimitsLegacy = async (taskArgs: TaskArguments, { ethers }: HardhatRuntimeEnvironment) => {
  ...
  const oftAdapter = await ethers.getContractAt('IEfficientRateLimiterV1', oappAddress);

  const limits: IEfficientRateLimiterV1.RateLimitConfigStruct[] = eids.split(',').map((eid: string) => {
    const e = BigInt(eid);
