...
async function checkOwnable({ ethers }: HardhatRuntimeEnvironment, contract: Contract, admins: AdminBucket) {
...
async function checkOwnable2Step({ ethers }: HardhatRuntimeEnvironment, contract: Contract, admins: AdminBucket) {
...
async function checkProxyAdminOwnership(
  ...
  const proxyAdminAddr = await upgrades.erc1967.getAdminAddress(await contract.getAddress());
...
async function checkAccessControlAdmin({ ethers }: HardhatRuntimeEnvironment, contract: Contract, admins: AdminBucket) {
  if (!contract.interface.hasFunction('DEFAULT_ADMIN_ROLE') || !contract.interface.hasEvent('RoleGranted')) return;

  // it's enough to check only address who get the role
  const events = await queryEventsSafe('RoleGranted', contract, ethers.provider);
  ...
  if (Object.keys(list).length === 0) {
    console.log('\t\tno RoleGranted events found, skip AccessControl check');
  }

  for (const addr in list) {
    const hasRole = await contract['hasRole'](contract['DEFAULT_ADMIN_ROLE'](), addr);
...
async function queryEventsSafe(event: string, contract: Contract, provider: HardhatEthersProvider) {
  let events;
  try {
    ...
    events = await contract.queryFilter(contract.getEvent(event), latest - range, latest);
  }
