...
export async function transferAccessControl(taskArgs: TaskArguments, hre: HardhatRuntimeEnvironment) {
  const { ethers } = hre;
  ...
  const tx2 = await contract.revokeRole(ethers.ZeroHash, await signer.getAddress());
  await tx2.wait(2);
